...
function PlanCard({
  ...
  const { data: planDetail } = useGetWorkoutPlan(plan.id, { query: { enabled: expanded } as any });
...
function EditExerciseForm({ exercise, planId, onClose }: { exercise: any; planId: number; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold mb-1">Tags</p>
        <div className="flex flex-wrap gap-1">
          {CATEGORIES.map((cat) => (
            <button key={cat} onClick={() => toggleTag(cat)}
              className={cn("text-[10px] font-bold px-2 py-1 rounded-lg transition-colors",
      ...
      <input type="url" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="Video URL (optional)"
        className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary/50" />
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div>
          <p className="text-[10px] text-muted-foreground uppercase mb-1">Reps</p>
          <input type="number" value={reps} onChange={(e) => setReps(Number(e.target.value))} min={1}
            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-center" />
        </div>
        <div>
          <p className="text-[10px] text-muted-foreground uppercase mb-1">Rest(s)</p>
          <input type="number" value={rest} onChange={(e) => setRest(Number(e.target.value))} min={0}
            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-center" />
        </div>
      </div>
      ...
      <button onClick={save} disabled={!name.trim() || updateExercise.isPending}
        className="w-full bg-primary text-primary-foreground py-2.5 rounded-xl font-bold text-sm disabled:opacity-40">
...
function NewExerciseForm({ planId, onClose }: { planId: number; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <p className="text-xs font-black tracking-widest text-primary uppercase">New Exercise</p>
        <button onClick={onClose} className="text-muted-foreground"><X className="w-4 h-4" /></button>
      </div>
      ...
      <button onClick={submit} disabled={!name.trim() || createExercise.isPending}
        className="w-full bg-primary text-primary-foreground py-2.5 rounded-xl font-bold text-sm disabled:opacity-40">
...
function ActiveWorkoutView({ session }: { session: any }) {
  ...
  const toggleSkip = useCallback((exId: number) => {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setActiveSec((p) => p), 1000);
    return () => clearInterval(id);
  }, [restActive]);
  ...
  const handleSetComplete = useCallback((restSeconds: number) => {
    ...
    const timeout = setTimeout(() => {
      setRestActive(false);
      setBreakSec((prev) => prev + restSeconds);
          ...
          {exercises.map((ex: any) => {
            ...
            return (
              <ExerciseTracker
                ...
                onSetComplete={() => handleSetComplete(ex.restSeconds || 60)}
